...
* **`Status`**: HP, MP, 공격력, 방어력 등 모든 스탯 데이터를 처리합니다. 기본 스탯과 장비 착용으로 인한 **보너스 스탯(`bonus_status`)**을 분리하여 관리합니다.

### **Item System**
* **`Item`**: 모든 아이템의 최상위 추상 클래스입니다. 순수 가상 함수인 `Use(Player* player)`를 통해 **다형성(Polymorphism)**을 제공합니다.
* **`Equipment`**: 무기, 방어구 등의 장비 아이템입니다. 장착 가능한 슬롯(`EquipmentSlot`) 정보와 보너스 스탯을 가집니다. `Use()` 호출 시 플레이어의 장착/해제 상태를 토글(Toggle)하고 스탯을 즉시 재계산합니다.
* **`Consumable`**: 포션 등 소모성 아이템입니다. `Use()` 호출 시 플레이어의 소모품을 사용합니다.
* **`JunkItem`**: 전투나 사용 목적이 없는 상점 판매용 잡템입니다.
...
- **전투 시스템**: D&D 식 다이스로 데미지 판정(데미지에 랜덤성 부여)
...
* 📝 [Item 시스템, 장비 장착시 보너스 스탯, UI 오류 해결](./Study/file4.md)